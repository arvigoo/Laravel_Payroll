...
// --- LOGIC ANTI-GHOST REQUEST ---
const updateManualGaji = (pay) => {
    if (!pay.id) return;

    const url = route('payroll.update', { payroll: pay.id });
    console.log("🚀 Mengupdate Gaji ID:", pay.id, "->", url);
    ...
    router.patch(url, {
        ...
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => console.log("✅ Berhasil Simpan"),
    ...
    <AppLayout title="Payroll Management">
        <template #header>
            <div class="flex justify-between items-center px-4">
                <h2 class="font-black text-2xl text-slate-800 tracking-tighter uppercase">Payroll Engine</h2>
        ...
        <div class="py-12 bg-[#f8fafc] min-h-screen">
            <div class="max-w-[100rem] mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex p-1 bg-slate-200/60 backdrop-blur-md rounded-2xl w-fit mb-8 border border-white shadow-inner">
                    ...
                    <button @click="activeSheet = 'Direksi'" :class="activeSheet === 'Direksi' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-500 hover:text-slate-700'" class="px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300">Direksi</button>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                    <div :class="activeSheet === 'Direksi' ? 'from-slate-800 to-slate-900 shadow-slate-200' : 'from-indigo-600 to-purple-700 shadow-indigo-100'" class="bg-gradient-to-br p-8 rounded-[3rem] shadow-2xl text-white transition-all duration-500">
                        <div class="text-[10px] opacity-60 uppercase tracking-[0.2em] font-black mb-1">Total Net Payroll ({{ activeSheet }})</div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[1800px]">
                        <thead>
                            <tr class="bg-slate-800/5 text-[10px] uppercase tracking-tighter font-black text-slate-500 border-b border-slate-100">
                                ...
                                <th rowspan="2" class="p-6 border-r border-slate-100 text-center">NIK</th>
                                <th colspan="3" class="p-3 border-r border-slate-100 text-center bg-blue-50/50 text-blue-600">Earnings</th>
                                <th colspan="5" class="p-3 border-r border-slate-100 text-center bg-orange-50/50 text-orange-600">BPJS Perusahaan</th>
                                <th rowspan="2" class="p-6 border-r border-slate-100 text-right bg-slate-900 text-white shadow-xl">Gaji Bruto</th>
                                <th colspan="4" class="p-3 border-r border-slate-100 text-center bg-red-50/50 text-red-600">Potongan Karyawan</th>
                            ...
                            <tr class="bg-slate-50/50 text-[9px] uppercase font-black text-slate-400 border-b border-slate-100 text-right">
                                ...
                                <th class="p-3 border-r border-slate-100 font-bold uppercase">Kes</th>
                        ...
                        <tbody class="text-[11px]">
                            <tr v-for="(pay, index) in filteredPayrolls" :key="pay.id" class="hover:bg-slate-50/80 transition-all border-b border-slate-100 group">
                                ...
                                <td class="p-4 border-r border-slate-50 text-center font-mono text-slate-400">{{ pay.employee.nik }}</td>
                                
                                <td class="p-4 border-r border-slate-50 text-right font-bold">
                                    <template v-if="activeSheet === 'Direksi'">
                                        <input 
                                            type="number" 
                                            v-model="pay.salary_pokok" 
                                            @change="updateManualGaji(pay)" 
                                            @keydown.enter.prevent
                                            class="w-full bg-slate-100/50 border-none rounded-lg text-right text-[11px] font-black focus:ring-2 focus:ring-indigo-500 h-8"
                                        >
                                    </template>
                                    <template v-else>{{ formatRp(pay.salary_pokok) }}</template>
                                </td>
                                <td class="p-4 border-r border-slate-50 text-right text-slate-400">{{ activeSheet === 'Direksi' ? 'Rp 0' : formatRp(pay.allowances) }}</td>
                                <td class="p-4 border-r border-slate-100 text-right font-black bg-blue-50/30 text-blue-800">{{ formatRp(parseFloat(pay.salary_pokok) + (activeSheet === 'Direksi' ? 0 : parseFloat(pay.allowances))) }}</td>
                                ...
                                <td class="p-4 border-r border-slate-100 text-right font-black bg-slate-800 text-white shadow-lg">{{ formatRp(pay.salary_pokok * 1.1089) }}</td>

                                <td class="p-4 border-r border-slate-50 text-right text-red-300 italic">{{ activeSheet === 'Direksi' ? 'Rp 0' : formatRp(pay.salary_pokok * 0.02) }}</td>
                                <td class="p-4 border-r border-slate-50 text-right text-red-300 italic">{{ activeSheet === 'Direksi' ? 'Rp 0' : formatRp(pay.salary_pokok * 0.01) }}</td>
                                <td class="p-4 border-r border-slate-50 text-right text-red-300 italic">{{ activeSheet === 'Direksi' ? 'Rp 0' : formatRp(pay.salary_pokok * 0.01) }}</td>
                                <td class="p-4 border-r border-slate-100 text-right font-black text-red-700 bg-red-50/30">-{{ activeSheet === 'Direksi' ? 'Rp 0' : formatRp(pay.bpjs_employee) }}</td>
                                ...
                                <td class="p-4 border-r border-slate-100 text-right text-red-600 font-black">
                                    <template v-if="activeSheet === 'Direksi'">
                                        <input 
                                            ...
                                            @change="updateManualGaji(pay)" 
                                            @keydown.enter.prevent
                                            class="w-full bg-red-50 border-none rounded-lg text-right text-[11px] font-black text-red-600 focus:ring-2 focus:ring-red-500 h-8"
                                        >
                                    </template>
                                ...
                                <td class="p-4 text-right font-black bg-emerald-500 text-white shadow-inner text-xs">
                                    {{ formatRp(parseFloat(pay.salary_pokok) + (activeSheet === 'Direksi' ? 0 : parseFloat(pay.allowances)) - parseFloat(pay.pph21) - (activeSheet === 'Direksi' ? 0 : parseFloat(pay.bpjs_employee))) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
